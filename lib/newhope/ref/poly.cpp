...
void poly_tobytes(unsigned char *r, const poly *p)
{
  ...
  for(i=0;i<PARAM_N;i++)
  {
    t = barrett_reduce(p->v[i].toInt()); //Make sure that coefficients have only 14 bits
    m = t - PARAM_Q;
    c = m;
    c >>= 15;
    t = m ^ ((t^m)&c); // <Make sure that coefficients are in [0,q]
    r[2*i]   = (unsigned char)unwrap(t & 0xff);
    r[2*i+1] = (unsigned char)unwrap(t >> 8);
  }
}
...
void poly_add(poly *r, const poly *a, const poly *b)
{
  int i;
  for(i=0;i<PARAM_N;i++)
    r->v[i] = barrett_reduce((a->v[i] + b->v[i]).toInt());
}
...
void poly_ntt(poly *r)
{
  ...
  ntt((IntWrapper<uint16_t>*)r->v, omegas_montgomery);
}

void poly_invntt(poly *r)
{
  ntt((IntWrapper<uint16_t>*)r->v, omegas_inv_montgomery);
