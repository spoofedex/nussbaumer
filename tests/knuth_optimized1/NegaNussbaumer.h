  ...
  while(j > 0) {
    ...
    for(std::size_t sPart = 0; sPart < (m_ >> j); ++sPart) {
      ...
      for(std::size_t t = 0; t < (1u << j); ++t) {
        ...
        auto tmp = addRotatedPolynomial(trans[e], trans[f], k);
...
Polynomial<RingElt> NegaNussbaumer<RingElt>::inverseTransform(
  ...
  // Do the inverse FFT (through a DIF with unordered input)
  Transformed z(trans);
  for(std::size_t j = 0; j <= (n_ >> 1); ++j) {
    for(std::size_t sPart = 0; sPart < (m_ >> j); ++sPart) {
      ...
      for(std::size_t t = 0; t < (1u << j); ++t) {
        ...
        Polynomial<RingElt> eNew(r_), fNew(r_);
        eNew = z[e] + z[f];
        fNew = rotatePolynomial(z[e] - z[f], k);
        z[e] = eNew;
        z[f] = fNew;
      }
    }
